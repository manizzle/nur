...
async def _make_app():
    ...
    import oombra.server.app as app_mod
    from oombra.server.app import create_app
    from oombra.server.db import Database
...
async def test_upload_without_key_when_required():
    ...
    os.environ["OOMBRA_API_KEY"] = "test-secret-key"
    try:
        app = await _make_app()
...
async def test_upload_without_key_open_mode():
    ...
    os.environ.pop("OOMBRA_API_KEY", None)
    app = await _make_app()
    transport = ASGITransport(app=app)
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        resp = await client.post(
...
async def test_query_vendor_with_enough_contributors():
    ...
    try:
        ...
        os.environ.pop("OOMBRA_MIN_K", None)
